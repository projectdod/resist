...
function ResistServer(options) {
  ...
  this.config          = options.config       || new ResistConfig();

  this.httpProxyServer = httpProxy.createServer(function (req, res, proxy) {
    var cacheOptions = {
      "debug"        : self.debug,
      "type"         : self.config.getCache.cache_type,
      "cacheNodes"   : self.config.getCache.cache_nodes,
      "cacheTimeout" : self.config.getCache.cache_timeout,
      "cleanMemory"  : self.config.getCache.cache_purge 
    };
  ...
  this.httpProxyServer.listen(this.config.getPort());
  this.httpProxyServer.proxy.on('proxyError', this.proxyError);
  this.httpProxyServer.proxy.on('start', function (req, res, target) {
