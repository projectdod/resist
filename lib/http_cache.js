...
HttpCache.prototype.set = function (res) {
  ...
  // XXX: if we have a cache, for a 4xx or 5xx we really should
  // still serve that.
  if (!this.cacheOk()) {
    return;
  }
  ...
  if (bodyBuffer && bodyBuffer.length > 0) {
    ...
    if (this.memcached) {
      ...
      memcached.set(this.buildKey(), blob, 0, function(err, result) {
        if (err) {
          console.error(err);
        }
      });
    } else {
      cache[this.buildKey()] = blob;

      // This is a little strange, but we delete the cache here.
      setTimeout(function () {
        if (cache[this.buildKey()]) {
          delete cache[this.buildKey()];
        }
...
HttpCache.prototype.cacheOk = function (key) {
  ...
  if (this.getResponse()
      && (this.getResponse().headers['cache-control'] === 'no-cache'
...
HttpCache.prototype.isStale = function () {
  ...
  // XXX this needs to take into account the timeout
